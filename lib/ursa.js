...
function PublicKey(rsa) {
    ...
    function hashAndVerify(algorithm, buf, sig, encoding, padding, salt_len) {
        if ((padding === ursaNative.RSA_PKCS1_PSS_PADDING) ||
            (padding === true)) {
            ...
            return rsa.verifyPSSPadding(algorithm, buf, sig,
    ...
    function addPSSPadding(algorithm, buf, salt_len) {
        return rsa.addPSSPadding(algorithm, buf, salt_len);
    }

    function verifyPSSPadding(algorithm, buf, sig, salt_len) {
        return rsa.verifyPSSPadding(algorithm, buf, sig, salt_len);
    }
...
function PrivateKey(rsa) {
    ...
    function hashAndSign(algorithm, buf, bufEncoding, outEncoding,
        ...
        if ((padding === ursaNative.RSA_PKCS1_PSS_PADDING) ||
            ...
            buf = rsa.addPSSPadding(algorithm, buf,
                    (salt_len === undefined) ? ursaNative.RSA_PKCS1_SALT_LEN_HLEN : salt_len);
...
module.exports = {
    ...
    RSA_PKCS1_PSS_PADDING:      ursaNative.RSA_PKCS1_PSS_PADDING,
